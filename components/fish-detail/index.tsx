...
import { getOneEvent } from '@/api/api-event';
...
const FishDetailComponent = ({ params }: { params: { fishId: string } }) => {
    ...
    const FetchFishDetail = useCallback(async () => {
        ...
        try {
            ...
            const eventPhase = await getOneEvent(authCookie, fishDetail.data[0].event_id);
            if (fishDetail) {
                setFishData(fishDetail.data[0]);
                if (eventPhase.data[0].eventReg.eventRegPhase.event_reg_phase_code === 'content_upload_phase') {
                    setSubmitPhase(true);
                }
            }
        } catch (error) {
    ...
    const handleSubmitPhaseFalse = async () => {
        showMessage('Fase pengumpulan Link post instagram belum dibuka', 'info');
    };
    ...
    return (
        <div className="space-y-4">
            <div className="rounded-md pb-0 pt-8">
                    ...
                    <div className="flex w-full items-center justify-center gap-2">
                        ...
                        {editMode && (
                            <div className="flex w-full items-center justify-center gap-2">
                                ...
                                <button type="button" onClick={handleOpenModal} disabled={isLoading || !isDirty}>
                                    <div
